...
int main(int argc, char **argv)
{
  ...
  for (const auto &p : *input_cloud)
  {
    ...
    thisP.z = x1 * cos(yaw) + y1 * sin(yaw);
    thisP.x = -x1 * sin(yaw) + y1 * cos(yaw);
    thisP.y = z1;

    output_cloud1->push_back(thisP);
  }
  output_cloud2 = target_cloud;
  ...
  Eigen::AngleAxisf init_rotation(yaw_pre, Eigen::Vector3f::UnitY());
  ...
  //对转换后的目标点云着色（绿色）并可视化
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
      output_color(output_cloud1, 0, 255, 0);
  viewer.addPointCloud<pcl::PointXYZ>(output_cloud1, output_color, "output cloud0", v1);
  viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,
                                          2, "output cloud0");

  //对目标点云着色（红色）并可视化
  viewer.addPointCloud<pcl::PointXYZ>(output_cloud2, target_color, "target cloud", v2);
  viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,
                                          1, "target cloud");
  //对转换后的目标点云着色（绿色）并可视化
