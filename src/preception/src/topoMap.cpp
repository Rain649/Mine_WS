...
#include <perception/topoMap_Config.h>
...
public:
    ...
    int getVertexIndex(const int v) const
    {
        ...
        {
            middle = (left + right) / 2;
            if (v == VertexTable[middle].name)
                return middle;
            else if (v < VertexTable[middle].name)
                right = middle - 1;
            else
                left = middle + 1;
        }
    ...
    {
        if (NumVertex >= MaxVertex || getVertexIndex(v) != -1)
            return;
    ...
    void show_VertexInfo(const int v) const
    {
        ...
        cout << "Position: (" << VertexTable[p].position.x << ", " << VertexTable[p].position.y << ", " << VertexTable[p].position.z
             << ")" << endl;
...
public:
    ...
    {
        if (!clusterNum || !peakNum || !segmentationRadius)
            return 0;
        float res{0};
        ...
        *filtered_cloud += *target_cloud;
        ...
        if (!menu_Bool)
        {
            if (current_Vertex < target_Index)
                ...
                yaw_init = this_Edge.begin_Angle;
...
/*动态参数调节*/
void callback(perception::topoMap_Config &config, uint32_t level)
{
...
int main(int argc, char **argv)
{
    ...
    /*动态参数调节*/
    dynamic_reconfigure::Server<perception::topoMap_Config> server;
    dynamic_reconfigure::Server<perception::topoMap_Config>::CallbackType f;
