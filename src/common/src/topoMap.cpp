...
std::vector<std::array<double, 3>> TopoMap::get_path(const int &incomingVertex_id, const int &currentVertex_id, const int &connectingVertex_id)
{
    if (vertex_Umap.count(currentVertex_id) > 0)
    {
        if (vertex_Umap[currentVertex_id].path_Umap.count({incomingVertex_id, connectingVertex_id}) > 0)
            return *vertex_Umap[currentVertex_id].path_Umap[{incomingVertex_id, connectingVertex_id}];
        else
            printf("No path between %d, %d, %d\n", incomingVertex_id, currentVertex_id, connectingVertex_id);
    }
    else
        printf("Vertex %d not exists\n", currentVertex_id);
    return {};
}
...
std::array<double, 3> TopoMap::get_startPoint(const int &vertex1_id, const int &vertex2_id)
{
    if (vertex1_id == vertex2_id)
        return {};
    if (vertex_Umap.count(vertex1_id) == 0 || vertex_Umap.count(vertex2_id) == 0)
    ...
    Vertex v = vertex_Umap[vertex1_id];
    LinkedInfo lI = v.linkedInfo_Umap[vertex2_id];
    std::array<double, 3> ret = lI.startPoint;
    return ret;
}
...
#pragma omp section
        {
            ...
            if (!notExistIndex.empty())
            {
                ...
                for (size_t i = 0; i < notExistIndex.size(); ++i)
...
void operator>>(const YAML::Node &node, LinkedInfo &lI)
{
    ...
    YAML::Node startPoint_node = node["startPoint"];
    for (unsigned i = 0; i < startPoint_node.size(); ++i)
    {
        lI.startPoint[i] = startPoint_node[i].as<double>();
    }
...
void operator>>(const YAML::Node &node, Vertex &v)
{
    ...
    const YAML::Node &paths_Node = node["paths"];
    for (unsigned i = 0; i < paths_Node.size(); ++i)
    {
        const YAML::Node &path_Node = paths_Node[i];
        int incomingVertex_id = path_Node["incomingVertex_id"].as<int>();
        int connectingVertex_id = path_Node["connectingVertex_id"].as<int>();
        std::vector<std::array<double, 3>> *path(new (std::vector<std::array<double, 3>>));
        const YAML::Node &wayPoints_Node = path_Node["path"];
        std::array<double, 3> arr;
        for (unsigned j = 0; j < wayPoints_Node.size(); ++j)
        {
            for (unsigned k = 0; k < 3; ++k)
            {
                arr[k] = wayPoints_Node[j][k].as<double>();
            }
            path->push_back(arr);
        }
        v.path_Umap.insert({{incomingVertex_id, connectingVertex_id}, path});
    }
    ///* 打印测试
    // for (auto i : v.path_Umap)
    // {
    //     printf("v.id: %d from %d to %d\n", v.id, i.first.first, i.first.second);
    //     for (auto j : *i.second)
    //     {
    //         for (auto k : j)
    //         {
